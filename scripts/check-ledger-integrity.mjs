...
const { checkArgs, sstArgs } = extractSharedSstArgs(forwardedArgs);
const isHelpRequest = checkArgs.includes("--help") || checkArgs.includes("-h");
const shouldUseSstShell =
    !isHelpRequest && (sstArgs.length > 0 || !hasLinkedLedger(resolvedLocalEnv));
const tsxCommand = [
    "exec",
    "tsx",
    "scripts/check-ledger-integrity.ts",
    ...checkArgs,
];
