...
function getStageName(arg, nextArg) {
    if (arg === "--stage") {
        return nextArg;
    }
...
function extractSharedSstArgs(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg.startsWith("--stage=") || arg.startsWith("--config=")) {
            const parsedStageName = getStageName(arg);
    ...
    if (stageName && !hasTargetArg(purgeArgs)) {
        purgeArgs.push("--target", stageName);
    }
...
const { purgeArgs, sstArgs } = extractSharedSstArgs(forwardedArgs);
const isHelpCommand = purgeArgs.includes("--help") || purgeArgs.includes("-h");
const shouldUseSstShell =
    !isHelpCommand && (sstArgs.length > 0 || !hasLinkedLedger(resolvedLocalEnv));
const tsxCommand = [
    "exec",
    "tsx",
    "scripts/purge-workspace-changes.ts",
    ...purgeArgs,
];
