...
function hasCompleteLocalResetEnv(env) {
    return hasLinkedLedger(env);
}
...
const { sstArgs, resetArgs } = extractSharedSstArgs(forwardedArgs);
const shouldUseSstShell =
    sstArgs.length > 0 || !hasCompleteLocalResetEnv(resolvedLocalEnv);
const tsxCommand = ["exec", "tsx", "scripts/reset-database.ts", ...resetArgs];
