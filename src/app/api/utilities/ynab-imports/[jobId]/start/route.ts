...
import { startYnabImportSchema } from "@/features/import/ynab/models/ynab-import-job";
...
export async function POST(
    ...
    return handleWorkspaceRoute(async (workspaceContext) => {
        const input = await parseJsonBody(request, startYnabImportSchema);
        const job = await beginYnabImport(
            jobId,
        ...
        try {
            await invokeYnabImportWorker({ action: "import", jobId });
        } catch (error) {
